...
export default function VideoPage() {
  ...
  const handleToggleFavored = useCallback(async () => {
    ...
    try {
      const lang = await storage.getLanguage() || "en";
      console.log(`[PlotTV-Fav] 收藏操作 block_key=${drama.block_key} lang=${lang} → ${newFavored ? '收藏' : '取消'}`);
      if (newFavored) {
        const res = await episodeApi.favoredEpisode(drama.block_key.toString(), lang as string);
        console.log(`[PlotTV-Fav] save_point 返回:`, JSON.stringify(res));
      } else {
