...
const ASSETS = {
  ...
  iconStoreLineLeft: require("@/assets/icons/icon_store_line_left.png"),
};
...
const SubscriptionItem = ({
  ...
  const getItemDisplayPrice = (product: MergedProduct) => {
  ...
  const getDiscountDescription = () => {
    ...
    return template.replace('{0}', getItemDisplayPrice(item)).replace('{1}', getOriginalPrice());
  };
        ...
        {isSpecial ? (
          <>
            ...
            <View style={styles.subMainRow}>
              <Text style={styles.subPriceTextLarge}>{getItemDisplayPrice(item)}</Text>
          ...
          <>
            <Text style={styles.subPriceText}>{getItemDisplayPrice(item)}/{intervalType === 'yearly' ? dict['Year'] || 'year' : intervalType === 'weekly' ? dict['Week'] || 'week' : dict['Month'] || 'month'}</Text>
            <View style={[styles.subMainRow, { marginTop: 8, marginBottom: 10 }]}>
...
const SKU = ({ onPurchaseSuccess, episodeId }: { onPurchaseSuccess?: () => void; episodeId?: string }) => {
  ...
  if (!isInitialized) {
    return (
      <View style={styles.centerContainer}>
        <ActivityIndicator style={{ marginTop: 12 }} size="small" color="#EE0000" />
      </View>
    );
  }
  ...
  const renderCoinItem = (item: MergedProduct) => {
    ...
    const bonusAmount = serverData.reward_gross;
    const giftRatio = (Number(serverData.premium_grade) * 100).toFixed(0);
    const isSelected = selectedCoin === serverData.unique_mark;

    const handleCoinItem = (coinItem: MergedProduct) => {
      setSelectedCoin(serverData.unique_mark);
      requestPurchaseByMergedProduct(coinItem, episodeId, onPurchaseSuccess);
    };

    return (
      <TouchableOpacity
        key={serverData.unique_mark}
        style={[styles.coinItem, isSelected && styles.coinItemSelected, loading && { opacity: 0.5 }]}
        activeOpacity={0.7}
        disabled={loading}
        onPress={() => handleCoinItem(item)}
      >
        ...
        <View>
          ...
          {Number(giftRatio) > 0 && <Text style={styles.coinBadgeText}>{'Save'} {giftRatio}%</Text>}
        </View>
      </TouchableOpacity>
    );
  };
            ...
            {coinProducts.length > 0 && (
              <>
                <View style={styles.dividerContainer}>
                  <Image
                    source={ASSETS.iconStoreLineLeft}
                    style={styles.dividerImage}
                  />
                  <Text style={styles.dividerText}>{dict['OR BUY COINS'] || 'OR BUY COINS'}</Text>
                  <Image
                    source={ASSETS.iconStoreLineLeft}
                    style={styles.dividerImage}
                  />
      ...
      {loading && (
        <View style={styles.loadingOverlay}>
          <View style={styles.loadingBox}>
            ...
            <Text style={styles.loadingOverlayText}>{dict['Loading...'] || 'Loading...'}</Text>
          </View>
        </View>
      )}
    </View>
